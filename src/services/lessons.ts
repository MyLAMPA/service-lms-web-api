...
export async function getLessons(
    ...
    let lessonsExec = db.lessons.find(params)

    const fieldsToPopulate = []
    if (populateLocation) {
        fieldsToPopulate.push('location')
    }
    if (populateCourse) {
        fieldsToPopulate.push('course')
    }
    if (populateGroup) {
        fieldsToPopulate.push('group')
    }
    if (populateTeachers) {
        fieldsToPopulate.push('teachers')
    }
    if (populateStudents) {
        fieldsToPopulate.push('students')
    }
    
    if (fieldsToPopulate.length > 0) {
        lessonsExec = lessonsExec.populate(fieldsToPopulate.join(' '))
    }

    const lessons = await lessonsExec.lean()
    return lessons
}
...
export async function getLesson(params: object, sort: object, state: State): Promise<Lesson> {
    let lessonsExec = db.lessons.find(params)
    if (!_.isNil(sort)) {
        lessonsExec = lessonsExec.sort(sort)
    }
    const lessons = await lessonsExec.limit(1).lean()
...
export async function countLessons(params: object, state: State): Promise<number> {
    ...
    const lessonsCount = await db.lessons.count(query)
    return lessonsCount
}

export async function getLessonById(lessonId: string, state: State): Promise<Lesson> {
    const lesson = await db.lessons.findById(lessonId).select('-attendance').lean()
    if (!_.isNil(lesson)) {
        return lesson
    }
